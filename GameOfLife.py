...
class GameOfLife:
    ...
    def on_aging_toggle(self):
        if self.aging_enabled.get() and not self.aging_was_enabled:
            for y in range(self.height):
                for x in range(self.width):
                    if self.grid[y][x]:
                        self.age_grid[y][x] = 0
        self.aging_was_enabled = self.aging_enabled.get()
            ...
            for x in range(self.width):
                ...
                if self.grid[y][x]:  # Клетка жива
                    if (neighbors < self.loneliness_threshold.get() or
                        ...
                        if self.aging_enabled.get():
                            self.age_grid[y][x] += 1
                            if self.age_grid[y][x] >= self.death_aging_slider.get():
                                new_grid[y][x] = 0
                                self.age_grid[y][x] = 0
                        else:
                            self.age_grid[y][x] = 0  # при отключенном старении просто не накапливаем возраст
